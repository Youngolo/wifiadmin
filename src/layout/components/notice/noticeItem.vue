...
function ToEvent() {
  try {
    let array, res = props.noticeItem;
    array = res.query.split('|');
    switch (res.fun) {
      case 'page':
        if (array[0] == '') {
          return;
      ...
      case 'tips':
        ...
        break;
      case 'msg':
        message(array[0] ? array[0] : "未知", { type: array[1] ? array[1] : "info" });
        break;
      default:
        console.log('没有该事件：', res.fun);
        break;
    }
  } catch (error) {
    console.log('事件执行失败');
...
<template>
  <div class="notice-container border-b-[1px] border-solid border-[#f0f0f0] dark:border-[#303030]" @click="ToEvent">
